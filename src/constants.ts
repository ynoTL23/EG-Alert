 ...
 * "Thursday 11:00 AM Eastern" — `shouldSendNow()` in `schedule.ts` enforces them
 ...
 * crons are UTC-only and UTC has no DST, so no single expression means "11:00
 * Eastern" year-round. We register both candidate UTC hours (15:0x = EDT summer,
 * 16:0x = EST winter) and let the guard discard whichever firing is not 11:00
 * Eastern today.
 *
